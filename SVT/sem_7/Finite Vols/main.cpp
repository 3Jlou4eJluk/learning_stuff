...
{
    // Establishing tags
    tagConc = m.DefineTag(tagNameConc, DATA_REAL, CELL, NONE, 1);
    tagD = m.DefineTag(tagNameD, DATA_REAL, CELL, NONE, 3);
    tagBCtype = m.DefineTag(tagNameBCtype, DATA_INTEGER, FACE, FACE, 1);
    tagBCval = m.DefineTag(tagNameBCval, DATA_REAL, FACE, FACE, 1);
    tagSource = m.DefineTag(tagNameSource, DATA_REAL, CELL, CELL, 1);
    tagConcAn = m.DefineTag(tagNameConcAn, DATA_REAL, CELL, CELL, 1);
    tagGlobInd = m.DefineTag(tagNameGlobInd, DATA_INTEGER, CELL, NONE, 1);

    // Iterating over cells
    int global_index = 0;
    for(auto cell_iter = m.StartCell(); cell_iter != m.FinishCell(); ++cell_iter){
        Cell cell = cell_iter->self();
        auto& cellDArray = cell.RealArray(tagD);
        cellDArray[0] = dx; // Dx component of the diffusion tensor
        cellDArray[1] = dy; // Dy component of the diffusion tensor
        cellDArray[2] = dxy; // Dxy component (off-diagonal)
        double cell_center[2];
        cell.GeometricCenter(cell_center);
        cell.Real(tagConcAn) = analyticalConcentration(cell_center[0], cell_center[1]);
        cell.Real(tagSource) = sourceTerm(cell_center[0], cell_center[1]);
        cell.Integer(tagGlobInd) = global_index++;
    }

    // Going through each face:
    for(auto face_iter = m.StartFace(); face_iter != m.FinishFace(); ++face_iter){
        Face face = face_iter->self();
        if(!face.OnBoundary())
            continue;
        face.Integer(tagBCtype) = BC_DIR_TYPE;
        double face_center[2];
        face.GeometricCenter(face_center);
        face.Real(tagBCval) = boundaryCondition(face_center[0], face_center[1]);
    }
}
