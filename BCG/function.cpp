...
int FillMatrix(double** matr, double *b, int n, int k, int rank, int size)
{
    ...
    for (i = 0; i < n; i++) {
        if (rank == i%size) {
            b[i/size] = 0;
            for (j = 0; j < n; j++) {
                matr[i/size][j] = formula(k, n, i, j); //+1 +1
                if (j % 2 == 0) {
                    t = matr[i/size][j];
                    b[i/size] += t;
                }
...
int ReadMatrix(double** matr, int n, char* filename, int rank, int size, double *buf1){
    double t;
    int d, i, j, c = 0;
    MPI_Status status;
    FILE* fin;
...
void bicg(double** A, double** A_t, double* b, double* x0, int n, int /*m*/, double eps, int max_iter, int my_rank, int size){
   double* tmp_vec1 = nullptr, *tmp_vec2 = nullptr, *x = nullptr;
    ...
    memset(tmp_vec2, 0, sizeof(double));
    if(my_rank == 0){
        if (x0 != NULL) {
            for (i = 0; i < n; i++) {
                x[i] = x0[i];
