...
void Problem::prepareSimulation() {
    // Инициализация тегов
    concentrationTag = model.CreateTag(concentrationTagName, DATA_REAL, NODE, NONE, 1);
    diffusionTag = model.CreateTag(diffusionTagName, DATA_REAL, CELL, NONE, 3);
    boundaryTypeTag = model.CreateTag(boundaryTypeTagName, DATA_INTEGER, NODE, NODE, 1);
    boundaryValueTag = model.CreateTag(boundaryValueTagName, DATA_REAL, NODE, NODE, 1);
    sourceTermTag =  model.CreateTag(sourceTagName, DATA_REAL, NODE, NONE, 1);
    analyticConcTag =  model.CreateTag(analyticConcentrationTagName, DATA_REAL, NODE, NONE, 1);
    indexTag = model.CreateTag(globalIndexTagName, DATA_INTEGER, NODE, NONE, 1);

    // Цикл по ячейкам
    forEachCell(m, [](Cell& cell) {
        if (!isTriangle(cell)) {
            cout << "Ошибочка: Ячейка не треугольник!" << endl;
            exit(1);
        }

        auto& diffVals = cell.RealArray(diffusionTag);
        diffVals[0] = diffX;
        diffVals[1] = diffY;
        diffVals[2] = crossDiff;
    });

    // Цикл по узлам сетки
    boundaryNodesMarker = model.CreateMarker();
    countBoundaryNodes = 0;
    int indexCounter = 0;
    forEachNode(m, [&](Node& node) {
        // Координаты узла для аналитического решения и источника
        double coord[3];
        node.Barycenter(coord);
        node.Real(analyticConcTag) = analyticalSolution(coord[0], coord[1]);
        node.Real(sourceTermTag) = sourceFunction(coord[0], coord[1]);

        // Проверка на граничный узел и установка параметров
        if (node.isOnBoundary()) {
            node.SetMarker(boundaryNodesMarker);
            node.Integer(boundaryTypeTag) = DIRICHLET_CONDITION;
            countBoundaryNodes++;
            node.Real(boundaryValueTag) = node.Real(analyticConcTag);
        } else {
            node.Integer(indexTag) = indexCounter++;
        }
    });

    printf("Количество граничных узлов: %d\n", countBoundaryNodes);
}

// вспомогательные функции
void forEachCell(Mesh& msh, std::function<void(Cell&)> action) {
    for(auto it = msh.BeginCell(); it != msh.EndCell(); ++it) {
        Cell cell = it->getAsCell();
        action(cell);
    }
}

void forEachNode(Mesh& msh, std::function<void(Node&)> action) {
    for(auto it = msh.BeginNode(); it != msh.EndNode(); ++it) {
        Node node = it->getAsNode();
        action(node);
    }
}

bool isTriangle(const Cell& cell) {
    return cell.getNodes().size() == 3;
}
...
void Problem::assembleLocalSystem(const Cell &c, rMatrix &A_loc, rMatrix &rhs_loc)
{
    ...
    for(unsigned i = 0; i < 3; i++){
        ...
        for(unsigned j = 0; j < 3; j++){
            A_loc(i,j) = (basis_func_grad(c, nodes[j]).Transpose() * \
                          D * basis_func_grad(c, nodes[i]))(0,0) * c.Volume();
        }
...
void Problem::assembleGlobalSystem(Sparse::Matrix &A, Sparse::Vector &rhs)
{
    ...
    for(Mesh::iteratorCell icell = m.BeginCell(); icell != m.EndCell(); icell++){
        ...
        for(unsigned loc_ind = 0; loc_ind < 3; loc_ind++){
            //glob_ind[loc_ind] = ???;
            unsigned result = nodes[loc_ind].Integer(tagGlobInd);
            glob_ind[loc_ind] = result;
        }
